package travel.travel.ControllerEs;
...
@RequestMapping("/api/toursEs")
@Slf4j
@RequiredArgsConstructor
public class TourControllerEs {
    ...
    public ResponseEntity<SimpleResponse> saveTour(@RequestBody TourRequest tourRequest) {
        SimpleResponse simpleResponse = tourService.saveTourEs(tourRequest);
    ...
    public ResponseEntity<TourResponseGetByID> getById(@PathVariable Long id) {
        TourResponseGetByID tourById = tourService.getTourByIdEs(id);
    ...
    public SimpleResponse updateTour(@PathVariable Long id,
        ...
        return tourService.updateTourEs(id, tourRequest);
    }
}